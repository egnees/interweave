...
//!
//! [`Observer::observe`] fires for every state the search reaches; [`Observer::step`]
//! fires at each discrete decision of the Optimal driver (descend, seed,
//! race-reversal, pop, …) and carries the borrowed [`Step`] / [`StepCx`] views below.
//! `step` is a no-op by default, so plain `explore` pays nothing: every emit site
//! passes a `Step<'_>` referencing data the driver already holds, and an observer that
//! ignores steps reads none of it. These step types are public so a visualizer is
//! built *on top of* the crate — a pure consumer of [`Observer::step`] plus the public
//! `model` surface, the same way any external consumer would.
